...
initial $readmemb("reg_data.mif",rf, 0, Nloc - 1);        	// Data to initialize registers
...
assign ReadData1 = (ReadAddr1 == {Abits {1'b0}}) ? {Dbits {1'b0}} :rf[ReadAddr1];                              	// First output port
assign ReadData2 = (ReadAddr2 == {Abits {1'b0}}) ? {Dbits {1'b0}} : rf[ReadAddr2];                              	// Second output port
